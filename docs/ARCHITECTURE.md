...
`MenuBarManager` isolates AppKit activation-policy changes behind a testable protocol. The app declares `LSUIElement` so background and scheduled launches never acquire a transient Dock item before the delegate runs. During launch, the persisted `hideFromDock` preference still selects `.accessory` to remain hidden or promotes an interactive app to `.regular` before launch finishes. An app-lifetime settings observation keeps that policy reversible even when no main window exists. The delegate retains launch work until application composition has installed the real `AppState` closures, preventing early lifecycle callbacks from running placeholders. Visible interactive launches and scheduled scans additionally retain their presentation request until composition installs the main-controller factory, then create and present the controller exactly once and yield one main-run-loop turn before maintenance or scan work begins so the window can paint. Hidden-Dock interactive launches create no initial window but can open the same controller from the menu bar. Scheduled signature updates never create a controller, remain in accessory mode, and run in a separate delegate-owned task that is not cancelled with a view lifecycle. Main-window title, identifier, size, style, and close/reopen behavior are AppKit-owned; repeated routes cannot create a second window.
