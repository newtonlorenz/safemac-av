...
single-instance Window + MenuBarExtra
     |
     +------ shared AppState
...
The app creates both a single-instance main `Window` and a persistent SwiftUI `MenuBarExtra` using the popover-like window style available on macOS 13 and later. Both scenes observe the same `AppState`, so protection, scan, and signature-update progress remain current when the main window is closed. The menu-bar surface can start a quick scan, update signatures, reopen the main window or its Settings tab, and quit the app. The single-instance scene deliberately removes the normal New Window behavior: startup and repeated menu-bar Open actions address the same main scene instead of racing to create duplicates.

`MenuBarManager` isolates AppKit activation-policy changes behind a testable protocol. The app declares `LSUIElement` so background and scheduled launches never acquire a transient Dock item before the delegate runs. During launch, the persisted `hideFromDock` preference still selects `.accessory` to remain hidden or promotes an interactive app to `.regular` before launch finishes so normal Dock behavior remains reversible at runtime. Because an agent app cannot rely on a scene task or menu-label task to run before the first window exists, the application delegate owns cold-start presentation. A representable embedded directly in the persistent menu-bar label installs SwiftUI's public `OpenWindowAction` into a one-shot lifecycle: a delegate request made before installation remains pending, installation made first remains ready, and concurrent requests coalesce into one open. The delegate first focuses the exact main identifier when it already exists; otherwise it requests that bridge and makes up to three bounded main-run-loop focus attempts before starting launch work. A main-scene marker applies the stable window identifier synchronously when its AppKit view joins the window, so unrelated onboarding, settings, import, utility, or alert windows cannot satisfy focus selection. Hidden-Dock interactive launches never request a window. Scheduled scans preserve their prior visible-window behavior, then run through the delegate-triggered application-lifetime lifecycle; interactive maintenance uses the same scene-independent route. Scheduled signature updates remain in accessory mode, suppress normal windows, and use their separate application-lifetime task that is not cancelled with a scene.
