...
`MenuBarManager` isolates AppKit activation-policy changes behind a testable protocol. The persisted `hideFromDock` preference selects `.accessory` to hide the Dock icon or `.regular` to restore it. On a hidden-Dock restart, an application delegate applies accessory mode before launch finishes and closes the automatically created initial main window on the next main-queue turn; the `MenuBarExtra` remains available and can create and activate the window on demand. The app deliberately does not set `LSUIElement`: Dock hiding stays reversible at runtime, and accessory apps can still be activated programmatically and present their normal windows.
