...
1. Change the app, Finder extension, and test bundle identifiers in `ClamAV-GUI.xcodeproj/project.pbxproj`. Upstream intentionally retains `com.newtonlorenz.ClamAV-GUI` for the main app's installed Sparkle update lineage; its Finder extension uses the required host-prefixed identity `com.newtonlorenz.ClamAV-GUI.SafeMacAV.FinderSync`.
2. Change `CQPH8YR62A.com.newtonlorenz.SafeMacAV` to an unprovisioned macOS app-group identifier prefixed by your signing Team ID, and add the matching entitlement to both participating targets.
